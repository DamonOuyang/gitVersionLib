...
void* func(void* arg)
{
	try
	{
		...
		/*******************第一种调用方式***************************/
		NlsRequest* request = nlc.createTtsRequest(callback, tst->config.c_str());					//创建一句话识别  NlsRequest，第一个参数为callback对象，第二个参数为config.txt文件
		/***********************************************************/

		/*******************第二种调用方式**************************
		NlsRequest* request = nlc.createTtsRequest(callback, NULL);
...
void OnResultDataRecved(NlsEvent* str,void* para=NULL)
{
	...
	std::cout << "Id:" << str->getId() << " " << str->getResponse() << endl;
}

void OnOperationFailed(NlsEvent* str, void* para=NULL)
{
	ParamStruct* tst = (ParamStruct*)para;
	std::cout << "Id:" << str->getId() << " " << str->getErrorMessage() << endl;
}
...
int main(int arc, char* argv[])
{
	try
	{
		...
		callback->setOnBinaryDataReceived(OnBinaryDataRecved);
		if (arc <= 7)
		{

...
void* func(void* arg)
{
	try
	{
		...
		/*******************第一种调用方???**************************/
		NlsRequest* request = nlc.createAsrRequest(callback, tst->config.c_str());					//创建一句话识别  NlsRequest，第一个参数为callback对象，第二个参数为config.txt文件
		/***********************************************************/

		/*******************第二种调用方???*************************
		  NlsRequest* request = nlc.createAsrRequest(callback, NULL);
